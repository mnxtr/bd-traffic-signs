...
def non_max_suppression(
    ...
    while len(boxes) > 0:
        ...
        # Calculate IoU with remaining boxes (only same class)
        ious = np.array([
            calculate_iou(current_box, box) if label == current_label else 0
            for box, label in zip(remaining_boxes, remaining_labels)
        ])
    ...
    original_indices = indices[keep]
    return boxes[keep], scores[keep], labels[keep] if len(keep) > 0 else (
...
def weighted_box_fusion(
    ...
    for label in unique_labels:
        ...
        for i in range(len(label_boxes)):
            ...
            for j in range(i + 1, len(label_boxes)):
                ...
                if iou > iou_threshold:
                    cluster.append((j, label_boxes[j], label_scores[j], label_model_indices[j]))
                    used[j] = True
        ...
        for cluster in clusters:
            ...
            if total_weight > 0:
                fused_box = (boxes_in_cluster * scores_in_cluster[:, np.newaxis]).sum(axis=0) / total_weight
            else:
    ...
    return fused_boxes[sorted_indices], fused_scores[sorted_indices], fused_labels[sorted_indices]
